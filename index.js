...
//     if (image) {
//       imageBase64 = image.buffer.toString('base64'); // Convert image buffer to base64 if a new image is uploaded
//     }
...
//       date,
//       join,
//       image: imageBase64, // Update image if a new one is provided
//     };
...
//       {  username },  // Search for the document using 'main'
  ...
  try {
    ...
    if (updatedRecords.length > 0) {
      ...
      for (const record of updatedRecords) {
        ...
        // // Example: Sending a notification or logging the floor/ward
        // console.log(`Record details - Floor: ${record.floor}, Ward: ${record.ward}`);
...
// API to check boards and send notifications
...
//       // Find tokens matching this floor, ward, and room
//       const matchedTokens = tokens.filter((t) => {
//         // Parse t.room from a stringified array into an actual array
//         let rooms;
//         try {
//           rooms = JSON.parse(t.room); // Parse the string into an array
//         } catch (error) {
//           console.error(`Error parsing t.room: ${t.room}`, error);
//           return false; // Skip invalid entries
//         }

//         // Check if room1 exists in the parsed rooms array and match floor and ward
//         return (
//           t.Selected==="floorInCharge"&&
//           t.floor === floor &&
...
//         // Send the notification to the first matching token
...
app.post('/api/boards/compare/:floor/:ward/:room', async (req, res) => {
  try {
    ...
    console.log("Requested rooms:", selectedRooms);  // Debugging the floor value
